...
- Delegate implementation only when it is parallelisable, multi-file, or context-heavy enough that a subagent's fresh context pays for itself. Sequential edits confined to one or two files are done DIRECTLY by the orchestrator — a fresh subagent starts cold and must re-read everything. A FORK (`/subtask`, or the `fork` agent type) inherits the full conversation and shares the prompt cache: prefer a fork for context-heavy parallel side-tasks where isolation is not the point, and a fresh subagent when it is. Parallel fast-workers that write files get per-call `isolation: worktree` (their edits land in a separate worktree branched from the default branch — integrate explicitly; nothing merges back automatically).
...
- Delegation specs are self-contained (fresh subagents do not see the conversation) and state: task and context, concrete file paths or line ranges (not descriptions of where to look), acceptance criteria ("run X, expect Y"), an explicit stop criterion or budget ("at most N tool calls" / "stop when X"), the skills/scripts to apply by name, and the return format. Batch small edits into one fast-worker call rather than one spin-up per edit.
- Decompose by shared context, not work type: each delegation owns a self-contained context bundle and integration happens at the orchestrator. Never chain subagent→subagent handoffs of partial work — relay chains (planner→implementer→tester) degrade like a telephone game.
- Verify fast-worker output by executing it, not by reading the diff; a "passed" claim without accompanying command output is unverified, and where a test suite exists require the full suite plus negative tests, not the first passing case. Treat deep-reasoner conclusions as claims, not facts; reject returns whose load-bearing claims lack file:line evidence, and spot-check the rest.
- Codex (codex@openai-codex plugin) is an independent peer from a different training lineage, not a reviewer of Claude output: `/codex:rescue --background <task>`, then collect with `/codex:status` and `/codex:result` (background jobs do not announce completion). Keep Codex read-only unless it is the sole writer on an isolated copy. Codex unavailability never blocks work: fail-warn and continue with Claude-only judgement; on rate limits, back off and collect later rather than retrying hot.
- High-stakes decisions (architecture choices with long-lived consequences, irreversible or client-facing changes, a bug that survived two failed fixes): blind parallel opinions. Write one neutral spec, note your own position first, send it identically to `deep-reasoner` and `/codex:rescue --background` in the same turn — neither sees the other's answer, or yours. Then run one cross-examination round: show each side the other's answer with authorship stripped and ask for critique — it filters weak arguments before you commit. The Claude side runs Opus 5 (same lineage as the orchestrator), so Codex is the only decorrelated brain in the duel — weight its dissent accordingly. Synthesise by adopting the stronger answer as the base and grafting improvements from the other; investigate disagreements before choosing. Not for routine work: it doubles cost by design.
...
- Agent teams (experimental) stay OFF globally by design: while the flag is set, any named subagent launches as a teammate — an independent session that never returns a result to the caller — so the wait-on-results delegations above can silently stall. For a genuine team task (parallel research/review with cross-challenge, competing-hypothesis debugging) start a dedicated session with `scripts/claude-team.ps1`; keep it to 3-5 teammates and budget for 3-4x token use.
